...
class RecursiveDivisionTest {
    ...
    void generateMaze2d5to5AndShow() throws IOException {
        ...
        final RecursiveDivision recursiveDivision = new RecursiveDivision(MazeDim.of(5, 5), random);
        //when
        final Maze maze = recursiveDivision.generateMaze();
        //then
        assertNotNull(maze);

        new MazeAsciiVisualizer(maze).printMaze();
        new MazeImageVisualizer(maze, 20, 2).saveMazeImage("target/recursive-division-1.png");
    ...
    void generateMaze2d20to20AndShow() throws IOException {
        //given
        final Random random = new SecureRandom();
        final RecursiveDivision recursiveDivision = new RecursiveDivision(MazeDim.of(20, 20), random);
        //when
        final Maze maze = recursiveDivision.generateMaze();
        //then
        assertNotNull(maze);
        ...
        new MazeAsciiVisualizer(maze).printMaze();
        new MazeImageVisualizer(maze, 20, 2).saveMazeImage("target/recursive-division-2.png");
